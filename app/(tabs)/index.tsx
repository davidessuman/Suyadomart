...
const generateProductWebLink = (product: Product): string => {
  const WEB_APP_DOMAIN = 'https://suyadomart.netlify.app';
        ...
        <View style={[styles.shareContainer, { backgroundColor: theme.modalBackground }]}>
          ...
          <View style={[styles.productLinkContainer, { 
            ...
            <Text style={[styles.productLinkExample, { color: theme.textTertiary }]}>
              https://suyadomart.netlify.app/?productId={product.id.substring(0, 8)}...
            </Text>
            <TouchableOpacity
              ...
              {displayMedia?.length > 0 && (
                <View style={[styles.mediaGalleryContainer, { alignItems: 'center' }]}>
                  <View style={styles.colorMediaSection}>
                    ...
                    {/* Show only one media at a time for ordered product details */}
                    {displayMedia && displayMedia.length > 0 && (
                      <TouchableOpacity
                        activeOpacity={0.95}
                        style={{ width: isLargeScreen ? Math.min(width * 0.8, 700) : '100%', height: isLargeScreen ? 420 : 260, alignSelf: 'center', backgroundColor: theme.background }}
                        onPress={() => onOpenFullViewer(0)}
                      >
                        {displayMedia[0].toLowerCase().includes('.mp4') ? (
                          // Show image thumbnail with play overlay for videos in ordered product details
                          (() => {
                            const thumb = getCardDisplayUrl(displayMedia);
                            return (
                              <View style={{ width: '100%', height: '100%' }}>
                                <Image source={{ uri: thumb }} style={{ width: '100%', height: '100%' }} resizeMode="cover" />
                                <View style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, justifyContent: 'center', alignItems: 'center' }}>
                                  <View style={{ width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' }}>
                          ...
                          <Image source={{ uri: displayMedia[0] }} style={{ width: '100%', height: '100%' }} resizeMode="contain" />
                        )}
                      </TouchableOpacity>
                    )}
...
const OrdersScreenModal: React.FC<{
  ...
  const renderOrderItem = ({ item }: { item: any }) => {
    ...
    const coverImage = getCardDisplayMedia(possibleMedia) || getCardDisplayUrl(possibleMedia) || 'https://ui-avatars.com/api/?name=Product&background=FF9900&color=fff';
      ...
      <View style={[styles.fullViewerContainer, { backgroundColor: theme.background }]}>
        ...
        <FlatList
          ...
          renderItem={({ item: url }) => {
            ...
            return (
              <View style={[styles.fullViewerMediaSlide, { backgroundColor: theme.background }]}>
                {isVideo ? (
                  <Video
                    source={{ uri: url }}
                  ...
                  <Image source={{ uri: url }} style={styles.fullViewerMediaImage} resizeMode="contain" />
                )}
              </View>
            );
          }}
              ...
              <View style={[styles.mediaGalleryContainer, { alignItems: 'center' }]}>
                {displayProduct.media_urls?.length > 0 && (
                  <TouchableOpacity 
                    ...
                    activeOpacity={0.9} 
                    onPress={() => onOpenFullViewer(currentMediaIndex)}
                  >
                    {displayProduct.media_urls[currentMediaIndex]?.includes('.mp4') ? (
                      <Video
                        source={{ uri: displayProduct.media_urls[currentMediaIndex] }}
                        style={{ width: '100%', height: '100%' }}
                        resizeMode={ResizeMode.CONTAIN}
                        shouldPlay={false}
                      />
...
const ProductFeedCard: React.FC<{
  ...
  const handleLikeToggle = async () => {
    ...
    setProducts(prev => prev.map(p =>
      p.id === item.id ? { ...p, isLiked: newLiked, likeCount: (p.likeCount || 0) + (newLiked ? 1 : -1) } : p
    ));
    try {
      if (newLiked) {
        const { error } = await supabase.from('product_likes').insert({ product_id: item.id, user_id: userId });
  ...
  return (
    <View style={{ height: ITEM_HEIGHT, width, backgroundColor: theme.background }}>
      ...
      <TouchableOpacity activeOpacity={1} style={[styles.mediaContainer, { alignItems: 'center', justifyContent: 'center' }]} onPress={handleDoubleTap}>
      ...
      <View style={[styles.rightSidebar, { bottom: insets.bottom + 90 }]}>
        ...
        <TouchableOpacity style={styles.sidebarItem} onPress={handleLikeToggle}>
          <Ionicons
      ...
      <View style={[styles.bottomInfoContainer, { width: width - 80, bottom: insets.bottom + 20 }]}>
...
const styles = StyleSheet.create({
  ...
  // Bottom Info
  bottomInfoContainer: { position: 'absolute', left: 18, zIndex: 10 ,marginBottom: 40},
  ...
  viewProductButton: { alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 7, borderRadius: 11, marginVertical: 7 },
  viewProductButtonText: { fontWeight: '900', fontSize: 13 },
  ...
  modalDiscountText: { color: '#fff', fontWeight: '900', fontSize: 13 },
