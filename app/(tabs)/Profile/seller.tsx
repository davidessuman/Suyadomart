...
function SellerDashboardContent() {
  ...
  const ProductsList = React.memo(({ sellerId, refreshTrigger }: { sellerId: string; refreshTrigger: number }) => {
    ...
    return <FlatList data={products} renderItem={renderProductItem} keyExtractor={(item) => item.id} contentContainerStyle={styles.productsListContainer} showsVerticalScrollIndicator={false} />;
  });
  ...
  const saveProduct = async () => {
    ...
    if (!editingProduct) {
      ...
      if (mainCategory === 'Services') {
        ...
        if (!requiresSizes && mainCategory !== 'Services' && productColors.length === 0 && (!generalStock || parseInt(generalStock) <= 0)) {
          showAlert({
            title: 'Required',
...
const FullImageViewer: React.FC<{
  ...
  const handleScreenTap = () => {
    ...
    if (currentUrl && currentUrl.toLowerCase().includes('.mp4')) {
      handlePlayPause();
    } else {
      toggleControls();
    }
  ...
  const currentUrl = mediaUrls[currentIndex];
  const isCurrentVideo = currentUrl?.toLowerCase().includes('.mp4');
            ...
            return (
              <TouchableOpacity style={viewerStyles.fullViewerMediaSlide} activeOpacity={1} onPress={handleScreenTap}>
                {isVideo ? (
                  ...
                  <Image source={{ uri: url || 'https://via.placeholder.com/600' }} style={viewerStyles.fullViewerMediaImage} resizeMode="contain" />
                )}
              </TouchableOpacity>
            );
          }}
